...
def run_bot(CONFIG, logging_level, hub_engine_path):
    ...
    lidraughts_bot.logger.info("Welcome {}!".format(username))
    ...
    if is_bot:
                ...
                while True:
                    ...
                    if len(board.move_stack) % 2 == 0:
                        if not board.move_stack:
                            ...
                            move = engine.play(board, draughts.engine.Limit(time=0.01), ponder=False)
        ...
        lidraughts_bot.logger.error("{} is not a bot account. Please upgrade it to a bot account!".format(user_profile["username"]))
...
def test_homemade():
    ...
    strategies += ["class Scan(ExampleEngine):", "    def __init__(self, commands, options, stderr, draw_or_resign, **popen_args):", "        super().__init__(commands, options, stderr, draw_or_resign, **popen_args)", f"        self.engine = hub_engine('./TEMP/scan{file_extension}')", "    def search(self, board, time_limit, *args):", "        return self.engine.play(board, time_limit, False)"]
